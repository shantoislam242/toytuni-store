...
- **Team page (`/admin/team`):** list all admins — DB rows + role badges; rows whose email ∈ env render a **"Permanent"** badge (cannot be removed/demoted from the UI). Actions: **Add admin** (email + role select), **change role**, **remove** — Super-gated; you cannot remove/demote **yourself**; env-bootstrap emails cannot be touched.
...
- **Team data/actions:** `getAdminTeam()` (queries.ts — DB rows + a `permanent: boolean` computed against the env list); actions `addAdminUser(email, role)` (validated email, lowercased, role enum, 23505 → "already an admin"), `setAdminRole(id, role)` (block if the row's email ∈ env, block self-demote), `removeAdminUser(id)` (block env rows + self) — ALL `getIsSuperAdmin()`-gated + service-role + `revalidatePath("/admin/team")`.
...
- Escalation guards: only Supers reach team actions; DB constraint restricts roles; self-demote/remove blocked server-side; add is lowercased + email-validated; RLS zero-policy keeps the table unreadable/unwritable publicly.
